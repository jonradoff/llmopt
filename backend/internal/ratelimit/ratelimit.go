...
// LLM Optimizer rate limit configs.
var (
	...
	LLMTestLimit        = RateLimitConfig{MaxRequests: 5, Window: time.Minute}
	PDFGenerateLimit    = RateLimitConfig{MaxRequests: 3, Window: 5 * time.Minute}
	BrandDiscoverLimit  = RateLimitConfig{MaxRequests: 10, Window: time.Minute}
	SummaryLimit        = RateLimitConfig{MaxRequests: 5, Window: time.Minute}
	APIReadLimit        = RateLimitConfig{MaxRequests: 60, Window: time.Minute}
	APIWriteLimit       = RateLimitConfig{MaxRequests: 30, Window: time.Minute}
)
...
func (rl *RateLimiter) Allow(key string, config RateLimitConfig) (bool, int, time.Time) {
	if rl.collection != nil {
		...
		log.Printf("WARN: distributed rate-limit check failed, falling back to local: key=%s error=%v", key, err)
	}
